...
class LoginClient(metaclass=SingletonMeta):
    ...
    def login_as_admin(self, base_url: str, username: str, password: str) -> str:
        ...
        try:
            ...
            self._log.error(f"Error during login request: {e}")
            raise Exception(f"Login request failed: {type(e).__name__}")

        if response.status_code == 201:
            cookie_headers = response.headers.get("Set-Cookie")
            cookie_headers_values = cookie_headers.split(";") if cookie_headers else []
            for cookie in cookie_headers_values:
